...
class Tools(object):
    ...
    def cookies():
        ...
        return cookies
    def url(headers,cookies,proxy):
        url=requests.get(url="https://store.steampowered.com/app/730/CounterStrike_Global_Offensive/",headers=headers,cookies=cookies,proxy=proxy)
        return url.text
    def page(url):
        page=r'\'^[);">](.+?)[</div>]$'
    ...
    def init():
        ...
        reader=Proxy.Read()
        for proxy in reader:
            url=Tools.url(headers,cookies,proxy[0])
        Tools.page(url)
